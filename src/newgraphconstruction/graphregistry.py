...
class GraphRegistry:
    ...
    def save_graphentry(self, graphname: str) -> None:
        ...
        For dynamic graphs:
        - Creates timestamp subdirectories
        - Saves edge_index_{timestamp}.pt in each
        - Saves edge_weight_{timestamp}.pt in each
        - Saves config.json at top level
        """
    ...
    def _save_static_graph(self, graph_entry: GraphEntry, graphname: str, directory: str) -> None:
        ...
        config_copy = {
            'graphname': graphname,
            'mode': 'static',
            'method': graph_entry.config.method,
            'self_connection': graph_entry.config.self_connection,
            'scaling_method': graph_entry.config.scaling_method,
            'kwargs': graph_entry.config.kwargs or {}
        }
    ...
    def _save_dynamic_graph(self, graph_entry: GraphEntry, graphname: str, directory: str) -> None:
        ...
        for t_idx, timestamp in enumerate(timestamps):
            ...
            snapshot_dir = os.path.join(directory, timestamp_str)
            ...
            # Save tensors
            torch.save(edge_index, os.path.join(snapshot_dir, f'edge_index.pt'))
            if edge_weight is not None:
                torch.save(edge_weight, os.path.join(snapshot_dir, f'edge_weight.pt'))
