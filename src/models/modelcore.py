...
class ModelCore:
    ...
    def forecast(self):
        """supposed to create the attribute `preds_df`"""
        raise NotImplementedError("Each model must implement its own forecast method.")
    
    def _process_predictions(self):
        # Aggregate over nodes: sum targets and preds by timestamp

        # nonormalized_evaluation_df = reverse_zscore_scaling(self.evaluation_df,columns = [self.target_column,'preds'], params =self.dataloader.norm_params)

        # agg_target = nonormalized_evaluation_df.groupby(self.temporal_column)[self.target_column].sum().reset_index()
        # agg_preds  = nonormalized_evaluation_df.groupby(self.temporal_column)['preds'].sum().reset_index()
    ...
    def show_forecasts(self,
        ...
        # Define GridSpec: 2 rows, 2 columns
        gs = gridspec.GridSpec(2, 2, height_ratios=[1, 1])  # both rows equal height
        # First subplot spans the full width
        ax1 = fig.add_subplot(gs[0, :])  # Row 0, all columns
        # Two subplots in second row
...
class DeepLearningModelCore(ModelCore):
    ...
    def _get_scheduler(self, scheduler_name: str, optimizer: torch.optim.Optimizer, scheduler_kwargs: Dict[str, Any]):
        """Factory method to create schedulers"""
        scheduler_map = {
    ...
    def _get_loss_function(self, loss_name: str):
        """Factory method to get loss functions"""
        loss_map = {
    ...
    def set_global_hparams(self, 
        ...
        if scheduler_kwargs is None:
            ...
            default_scheduler_kwargs = {
                ...
                'plateau': {'mode': 'min', 'patience': 5, 'factor': 0.5},
                'cosine': {'T_max': n_epochs},
                'multistep': {'milestones': [30, 80], 'gamma': 0.1}
            }
    ...
    def train(self, **kwargs):
        ...
        for epoch in range(self.n_epochs):
            ...
            if val_improved:
                ...
                patience_counter += 1
                print(f"Epoch {epoch} train MSE: {train_mse:.4f}, val MSE: {val_mse:.4f} (patience: {patience_counter}/{self.patience})")
