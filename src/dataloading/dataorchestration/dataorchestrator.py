...
class EpiFeatureBuilder:
    ...
    def _time_week_index(self, df: pd.DataFrame, shift_for_target: bool = False) -> pd.DataFrame:
        """
        ...
        ----------
        df : pd.DataFrame
        shift_for_target : bool
            If True, shift the week-of-year features forward by horizon_leadtime
            to align with the target timestamp
        """
        dfc = df.copy()
        
        if shift_for_target:
            # Calculate week-of-year for the TARGET timestamp
            # For weekly data, add horizon_leadtime weeks
            target_timestamps = dfc[self.config.temporal_column] + pd.Timedelta(weeks=self.config.horizon_leadtime)
            iso_calendar = target_timestamps.dt.isocalendar()
        else:
            # Calculate week-of-year for current timestamp
        ...
        if self.config.verbose > 1:
            ...
            print(f'{checkmark} weekly time index {sin_col}, {cos_col} included as feature ({alignment_msg})')
        return dfc
    
    def _time_day_index(self, df: pd.DataFrame, shift_for_target: bool = False) -> pd.DataFrame:
        """
        ...
        df : pd.DataFrame
        shift_for_target : bool
            If True, shift the day-of-week features forward by horizon_leadtime
            to align with the target timestamp
        """
        dfc = df.copy()
        
        if shift_for_target:
            # Calculate day-of-week for the TARGET timestamp (horizon_leadtime days ahead)
            target_timestamps = dfc[self.config.temporal_column] + pd.Timedelta(days=self.config.horizon_leadtime)
            iso_calendar = target_timestamps.dt.isocalendar()
        else:
            # Calculate day-of-week for current timestamp
            iso_calendar = dfc[self.config.temporal_column].dt.isocalendar()
        ...
        if self.config.verbose > 1:
            alignment_msg = "aligned with target" if shift_for_target else "for current timestamp"
            print(f'{checkmark} daily time index {sin_col}, {cos_col} included as feature ({alignment_msg})')
    ...
    def _lags(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        Timeline:
        - lag_column_t0: current timestep (timestamp)
        - lag_column_t_1: 1 timestep ago
        - lag_column_t_2: 2 timesteps ago
        - target: horizon_leadtime timesteps ahead
        """
        dfc = df.copy()
        
        # Step 1: Create t0 for the lag column (current value, unshifted)
        ...
        # Step 3: Shift target into the future
        ...
        # Step 4: Drop the original unshifted columns
    ...
    def _rename_lagt0(self, df: pd.DataFrame) -> pd.DataFrame:
        """Rename lag_column to lag_column_t0"""
        old_featurename = self.config.lag_column
        new_featurename = f'{self.config.lag_column}_t0'
        return df.rename(columns={old_featurename: new_featurename})
    ...
    def orchestrate(self, processed_data: 'ProcessedEpiData') -> 'FeatureEpiData':
        """Orchestrate entire feature addition with CORRECT timestamp alignment"""
        ...
        # CRITICAL FIX: Create lags FIRST, then add time features aligned with target
        ...
        if self.config.verbose:
            print(f'{checkmark}{checkmark} All features built with correct timestamp alignment') 
        if self.config.verbose > 1:
            print("")
...
class EpiDataFinalizer:
    ...
    def _add_horizons(self, df: pd.DataFrame) -> pd.DataFrame:
        """
        Loop over horizons, add shifted target under new name f'target_lead{steps_ahead}'
        where steps_ahead = horizon_leadtime + horizon_idx
        """
    ...
    def _create_target_timestamps(self, df: pd.DataFrame) -> pd.DataFrame:
        """
        Create timestamp columns that represent WHEN the prediction is FOR,
        not when it's made FROM.
        
        For each target_lead{X}, create target_timestamp_lead{X} that is
        ...
        if self.config.verbose > 1:
            print(f'{checkmark} target timestamps created for plotting alignment')
    ...
    def orchestrate(self, normalized_data: NormalizedEpiData) -> 'FinalizedEpiData':
        ...
        # CRITICAL: Create target timestamps for correct plotting
        dfc = self._create_target_timestamps(dfc)
        ...
        # Groundtruth uses the first target horizon with CORRECT timestamp
        groundtruth_df = dfc_nanfree.copy()[
            [f'timestamp', 'node', f'target_lead{base_lead}']
        ]
        groundtruth_df = groundtruth_df.rename(columns={
            f'target_lead{base_lead}': 'target'
        })
...
class DataOrchestrator:
    ...
    def preview(self, node_idx: int, status: str = 'processed') -> ManagedFigure: 
        """preview dataloader"""
        if status == 'processed':
