...
from .datastagecontainers import RawEpiData, ContextEpiData, ProcessedEpiData, FeatureEpiData, NormalizedEpiData, FinalizedEpiData
...
class EpiDataPreprocessor:
    ...
    def _split_epipopdata(self, epipopdata: pd.DataFrame) -> Tuple[pd.DataFrame, pd.DataFrame]:
        """selects columns (timestamp, node, incidence) and when necessary, population_size"""
        columns_epidata = ['timestamp','node','incidence']
        columns_popdata = ['timestamp','node','population_size']

        return epipopdata[columns_epidata], epipopdata[columns_popdata]
    ...
    def orchestrate(self, ContextEpiData: 'ContextEpiData') -> 'ProcessedEpiData':
        ...
        epidata, popdata= self._split_epipopdata(epipopdata)           

        return ProcessedEpiData(epidata = epidata, popdata = popdata)
...
class EpiFeatureBuilder:
    ...
    def _log_target(self, df: pd.DataFrame) -> pd.DataFrame:
        """log_transform target"""
        df_transformed                              = df.copy()
        df_transformed[self.config.target_column]   = np.log(df_transformed[self.config.target_column] + self.config.log_shift)
    ...
    def _population_size(self, epidata: pd.DataFrame, popdata: pd.DataFrame) -> pd.DataFrame:
        self.column_registration.add_column(
                'population_size', 
                'feature', 
                needs_normalization=True,
        ...
        return pd.merge(epidata, popdata, on = ['timestamp','node'])
    ...
    def _lags(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        """
        for lag in range(self.config.lags):  # 0 to lags-1
            feature = f'{self.config.target_column}_lag{lag}'
            df[feature] = df.groupby(self.config.id_column)[self.config.target_column].shift(lag)
    ...
    def orchestrate(self, processed_data: 'ProcessedEpiData') -> 'FeatureEpiData':
        ...
        feature_data = processed_data.epidata

        if self.config.include_population:
            feature_data = self._population_size(feature_data, processed_data.popdata)
            if self.config.verbose > 1:
        ...
        if self.config.log_transform:
            feature_data = self._log_target(feature_data)
...
class EpiDataFinalizer:
    ...
    def orchestrate(self, normalized_data: NormalizedEpiData) -> 'FinalizedEpiData':
        ...
        for horizon in range(self.config.horizon_size):
            ...
            target      = f'{self.config.target_column}_ahead{steps_ahead}'
        ...
        # Drop the base target column
        dfc = dfc.drop(labels=self.config.target_column, axis=1)
        if self.config.verbose > 1:
        ...
        return FinalizedEpiData(
            data=dfc,
            config=self.config,
            groundtruth= dfc[['timestamp','node','incidence_lag0']].rename(columns = {'incidence_lag0':'target'})
        ) 
